...
class EffEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        eff = efficientnet_b7(weights=EfficientNet_B7_Weights.DEFAULT).features
        self.slice1 = eff[:1]
        self.slice2 = eff[1:2]
        self.slice3 = eff[2:3]
        self.slice4 = eff[3:5]
...
class Decoder(nn.Module):
    def __init__(self): # b 160 16 16
        super().__init__()
        self.rc1 = RC(160, 160, 3, 1)
        self.rc2 = RC(160, 160, 3, 1)
        self.rc3 = RC(160, 160, 3, 1)
        self.rc4 = RC(160, 160, 3, 1)
        self.rc5 = RC(160, 80, 3, 1)
        self.rc6 = RC(80, 80, 3, 1)
        self.rc7 = RC(80, 64, 3, 1)
...
class Model(nn.Module):
    ...
    def forward(self, content_images, style_images, alpha=1.0, lam=10):
        content_features = self.vgg_encoder(content_images, output_last_feature=True)
        style_features = self.vgg_encoder(style_images, output_last_feature=True)
        t = adain(content_features, style_features) # b 160 16 16
        t = alpha * t + (1 - alpha) * content_features
