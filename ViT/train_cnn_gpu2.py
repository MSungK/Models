...
if __name__ == '__main__':
    ...
    exit()
    model.classifier = nn.Sequential(nn.Dropout(p=0.2, inplace=True), nn.Linear(1536, 345, bias=True))
    if multi_gpu == True:
        model = nn.DataParallel(efficientnet_b3(weights = weight), device_ids = [device])
        model.cuda()
    else:
        pass
    ...
    for epoch in range(1, max_epochs+1):
        ...
        if epoch % checkpoint == 0:
            ...
            if acc/total*100 >= best_val_acc:
                save_model(model, osp.join(save_path, f'effb3_epoch_{epoch}.pth'))
